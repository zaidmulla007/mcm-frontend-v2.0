...
export async function GET(request) {
  try {
    ...
    const externalApiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/rankingstelegramdata/specificFieldRankings?channel_id=${encodeURIComponent(channelId)}&fields=${encodeURIComponent(fields)}`;

    console.log('Fetching Telegram rankings data from:', externalApiUrl);
    ...
    console.log('Telegram rankings API response:', JSON.stringify(data, null, 2));
    ...
    console.error('Error fetching Telegram rankings data:', error);
    console.error('Error name:', error.name);
    console.error('Error message:', error.message);

    let errorMessage = 'Failed to fetch Telegram rankings data';
    let statusCode = 500;
