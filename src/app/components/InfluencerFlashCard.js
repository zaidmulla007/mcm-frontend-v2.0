...
export default function InfluencerFlashCard({ data, rank, isLoggedIn, onViewFull }) {
  ...
  const extractMoonshotProb = (moonshotData) => {
    const moonshotProb = {};

    if (moonshotData?.yearly) {
      Object.entries(moonshotData.yearly).forEach(([year, yearData]) => {
        ...
        // Only include years with data (has_data: true)
        if (yearData?.["180_days"]?.has_data) {
  ...
  const moonshotProb = data?.moonshotData ? extractMoonshotProb(data.moonshotData) : {};
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div className="relative">
              <span className="absolute -top-1 -left-1 bg-yellow-400 text-black font-bold text-xs rounded-full w-6 h-6 flex items-center justify-center z-10">
                #{rank}
              </span>
              {profileImage ? (
      ...
      <div className="p-4 space-y-4">
            ...
            {Object.keys(totalCalls).length > 0 ? (
              (() => {
                ...
                Object.entries(totalCalls).forEach(([quarter]) => {
                  ...
                  if (!yearlyData[year]) {
                    yearlyData[year] = { quarters: [] };
                  }
                  yearlyData[year].quarters.push({ quarter, winRate });
                });

                // Calculate max for scaling
                const allWinRates = Object.values(yearlyData).flatMap(yd =>
                  yd.quarters.map(q => q.winRate)
                );
                ...
                Object.entries(yearlyData).forEach(([year, yearData]) => {
                  const sum = yearData.quarters.reduce((acc, q) => acc + q.winRate, 0);
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex flex-col items-center min-w-[40px]">
                                <div className="text-[10px] font-bold mb-1 whitespace-nowrap text-purple-600">
                                  {avgWinRate.toFixed(1)}%
                                </div>
                                <div className="w-full flex items-end" style={{ height: '60px' }}>
                                  <div
                                    className="w-full bg-gradient-to-t from-purple-600 to-purple-400 rounded-t-sm cursor-pointer hover:from-purple-700 hover:to-purple-500 transition-colors"
                                    style={{ height: height > 0 ? `${height}%` : '0px', minHeight: avgWinRate > 0 ? '5px' : '0px' }}
                                    title={`${year} Avg: ${avgWinRate.toFixed(1)}%`}
                                  ></div>
                                </div>
                          ...
                          {yearData.quarters.sort((a, b) => a.quarter.localeCompare(b.quarter)).map(({ quarter, winRate }) => {
                            ...
                            return (
                              <div key={quarter} className="flex flex-col items-center min-w-[40px]">
                                ...
                                <div className="text-[10px] font-semibold mb-1 whitespace-nowrap text-green-600">
                                  {winRate.toFixed(1)}%
                                </div>
                                {/* Bar with gradient */}
                                <div className="w-full flex items-end" style={{ height: '60px' }}>
                                  <div
                                    className="w-full bg-gradient-to-t from-green-600 to-green-400 rounded-t-sm cursor-pointer hover:from-green-700 hover:to-green-500 transition-colors"
                                    style={{ height: height > 0 ? `${height}%` : '0px', minHeight: winRate > 0 ? '5px' : '0px' }}
                                    title={`${quarter}: ${winRate.toFixed(1)}%`}
                                  ></div>
                                </div>
                      ...
                      return (
                        <div key={year} className="flex items-end gap-2">
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex flex-col items-center min-w-[40px]">
                                <div className={`text-[10px] font-bold mb-1 whitespace-nowrap ${avgROI >= 0 ? 'text-purple-600' : 'text-red-600'}`}>
                                ...
                                <div className="w-full flex items-end" style={{ height: '60px' }}>
                                  <div
                                    ...
                                    title={`${year} Avg: ${avgROI >= 0 ? '+' : ''}${avgROI.toFixed(1)}%`}
                                  ></div>
                                </div>
                                <div className="text-[9px] text-gray-900 font-bold mt-1 text-center whitespace-nowrap">
                                  {year}
                          ...
                          {yearData.quarters.sort((a, b) => a.quarter.localeCompare(b.quarter)).map(({ quarter, roi }) => {
                            ...
                            return (
                              <div key={quarter} className="flex flex-col items-center min-w-[40px]">
                                ...
                                <div className={`text-[10px] font-semibold mb-1 whitespace-nowrap ${roi >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {roi >= 0 ? '+' : ''}{roi.toFixed(1)}%
                                </div>
                                {/* Bar with gradient */}
                                <div className="w-full flex items-end" style={{ height: '60px' }}>
                                  <div
                                    ...
                                    title={`${quarter}: ${roi >= 0 ? '+' : ''}${roi.toFixed(1)}%`}
                                  ></div>
                                </div>
                                {/* Quarter label below bar */}
                                <div className="text-[9px] text-gray-700 mt-1 text-center whitespace-nowrap">
        ...
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            {Object.entries(moonshotProb).map(([year, prob]) => (
