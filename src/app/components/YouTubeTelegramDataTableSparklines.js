...
export default function YouTubeTelegramDataSparklines({ useLocalTime: propUseLocalTime = false }) {
    ...
    const renderSparklineSection = (timeframe, title) => {
                        ...
                        {coins.length === 0 ? (
                            ...
                            coins.map((coin, index) => {
                                ...
                                const shortTermColor = shortTermPosts > 0
                                    ? (shortTermBullish >= shortTermBearish ? "bg-blue-50 border-blue-400" : "bg-red-50 border-red-400")
                                    : "bg-gray-50 border-gray-400";

                                const shortTermBarColor = shortTermPosts > 0
                                    ? (shortTermBullish >= shortTermBearish ? "bg-blue-500" : "bg-red-500")
                                    : "bg-gray-500";

                                const longTermColor = longTermPosts > 0
                                    ? (longTermBullish >= longTermBearish ? "bg-green-50 border-green-400" : "bg-red-50 border-red-400")
                                    : "bg-gray-50 border-gray-400";

                                const longTermBarColor = longTermPosts > 0
                                    ? (longTermBullish >= longTermBearish ? "bg-green-500" : "bg-red-500")
                                    : "bg-gray-500";

                                const shortTermTextColor = shortTermPosts > 0
                                    ? (shortTermBullish >= shortTermBearish ? "text-blue-600" : "text-red-600")
                                    : "text-gray-600";

                                const longTermTextColor = longTermPosts > 0
                                ...
                                return (
                                    <div key={index} className="bg-white p-4 rounded-lg border border-gray-200">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center gap-2">
                                                <img
                                                    ...
                                                    onError={(e) => {
                                                        ...
                                                        e.target.src = `https://ui-avatars.com/api/?name=${coin.symbol}&background=ED8936&color=fff&size=32`;
                                                    }}
                                                />
                                            ...
                                            <span className="text-xs text-gray-500">Total {sentimentData.mentions} posts</span>
                                        </div>
                                        <div className="flex items-center justify-center gap-6 mt-4">
                                            {/* Short-term sparkline representation */}
                                            <div className="flex flex-col items-center">
                                                <div className="text-center mb-1">
                                                    <div
                                                        className={`w-12 h-12 ${shortTermColor} rounded border-2`}
                                                        style={{
                                                            display: 'flex',
                                                            alignItems: 'flex-end',
                                                    ...
                                                    >
                                                        <div
                                                            className={`w-2 ${shortTermBarColor} rounded-t`}
                                                            style={{ height: `${(shortTermPercentage / 100) * 100}%` }}
                                                        />
                                                    </div>
                                                </div>
                                                <p className={`text-xs font-bold ${shortTermTextColor} mt-1`}>{shortTermPercentage}%</p>
                                                <p className="text-xs text-gray-500 mt-1">ST:{shortTermPosts > 0 ? `${shortTermPosts} posts` : "0 posts"}</p>
                                            </div>
                                            
                                            {/* Long-term sparkline representation */}
                                            <div className="flex flex-col items-center">
                                                <div className="text-center mb-1">
                                                    <div
                                                        className={`w-12 h-12 ${longTermColor} rounded border-2`}
                                                        style={{
                                                            display: 'flex',
                                                            alignItems: 'flex-end',
                                                            justifyContent: 'center',
                                                    ...
                                                    >
                                                        <div
                                                            className={`w-2 ${longTermBarColor} rounded-t`}
                                                            style={{ height: `${(longTermPercentage / 100) * 100}%` }}
                                                        />
                                                    </div>
                                                </div>
                                                <p className={`text-xs font-bold ${longTermTextColor} mt-1`}>{longTermPercentage}%</p>
                                                <p className="text-xs text-gray-500 mt-1">LT:{longTermPosts > 0 ? `${longTermPosts} posts` : "0 posts"}</p>
                                            </div>
                                        </div>
                                    </div>
                                );
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
                {renderSparklineSection("6hrs", "Last 6 Hours")}
                {renderSparklineSection("24hrs", "Last 24 Hours")}
                {renderSparklineSection("7days", "Last 7 Days")}
                {renderSparklineSection("30days", "Last 30 Days")}
            </div>
        </div>
    );
}