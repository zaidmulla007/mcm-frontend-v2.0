...
export default function YouTubeTelegramDataTableChips({ useLocalTime: propUseLocalTime = false }) {
    ...
    const renderChipSection = (timeframe, title) => {
                                ...
                                return (
                                    <div key={index} className="bg-white p-4 rounded-lg border border-gray-200">
                                        ...
                                        <div className="flex gap-2 flex-wrap">
                                            <div className={`flex items-center gap-1 ${shortTermColor} px-3 py-1.5 rounded-full min-w-[140px] flex-1`}>
                                                <span className="text-xs font-semibold text-gray-600">Short Term:</span>
                                                <span className="text-sm font-bold">{shortTermPercentage}%</span>
                                                <span className="text-xs text-gray-500 whitespace-nowrap ml-auto">{shortTermPosts} Posts</span>
                                            </div>
                                            <div className={`flex items-center gap-1 ${longTermColor} px-3 py-1.5 rounded-full min-w-[140px] flex-1`}>
                                                <span className="text-xs font-semibold text-gray-600">Long Term:</span>
                                                <span className="text-sm font-bold">{longTermPercentage}%</span>
                                                <span className="text-xs text-gray-500 whitespace-nowrap ml-auto">{longTermPosts} Posts</span>
                                            </div>
                                            <div className="flex items-center justify-center gap-1 bg-gray-100 px-3 py-1.5 rounded-full min-w-[100px] flex-1">
                                                <span className="text-xs font-semibold text-gray-600 whitespace-nowrap">Neutral: {Math.abs(shortTermPercentage - longTermPercentage)}%</span>
                                            </div>
                                        </div>
                                    </div>
                                );
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
                {renderChipSection("6hrs", "Last 6 Hours")}
                {renderChipSection("24hrs", "Last 24 Hours")}
                {renderChipSection("7days", "Last 7 Days")}
                {renderChipSection("30days", "Last 30 Days")}
            </div>
        </div>
    );
}